...
<div class="my-8 w-auto border-2 border-neutral-300 bg-neutral-300 dark:border-neutral-400 dark:bg-slate-800 sm:mx-4 md:my-14 lg:mx-auto lg:max-w-screen-lg">
	<h1 class="font-sans text-3xl font-bold dark:bg-neutral-900 px-4 py-0.5 dark:text-white">Posts</h1>
	<ul>
		{#each data.posts as post, i}
			<li class={`dark:bg-neutral-800 ${i % 2 === 0 ? 'bg-gray-100' : 'bg-gray-200'}`}>
				...
				>
					<div class="flex items-center">
						<h2
							class="font-sans text-2xl font-bold text-black dark:text-white group-hover:underline group-focus:underline sm:text-4xl"
						>
							{post.title}
						</h2>
           ...
           <PixelarticonsCalendar class="sm:h-6 sm:w-6 me-3 hidden sm:block text-gray-400 dark:text-neutral-300" />
            <span
              class={`block font-bold sm:text-lg sm:mt-px dark:text-neutral-400 ${i % 2 === 0 ? 'text-gray-500' : 'text-gray-600'}`}
            >
              Posted on
